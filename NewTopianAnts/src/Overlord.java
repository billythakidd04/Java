...
public class Overlord {
	...
	private enum Owner
	{
		MINE("Q"),//sets MINE as an object of Owner and its string value is Q
		ENEMY("@");//sets ENEMY as an object of Owner and its string value is @
		String value;
	...
	private enum Topology 
	{
		...
		public static Topology fromIlk(Ilk ilk) 
		{
			...
			switch (ilk) //if tile is anything but water set as "." other wise "#"
			{
			case DEAD:
			case ENEMY_ANT:
			case FOOD:
	...
	public Overlord(Ants ants) 
	{
		...
		this.longestDistance = (int) Math.round(Math.sqrt(row * row + col * col));//pythagorean theorem longestDistance = C where (A*A)+(B*B)=(C*C)

		this.viewRadius = (int) Math.round(Math.sqrt((double) ants.getViewRadius2()));//fog of war

		this.topologyOverlay = new Topology[col][row];//2d array... 
		...
		this.scentTrails = new int[col][row];//another 2d array
	...
	public List<SingleAnt> assessSituation() 
	{
		...
		//remainToEvaluate is a deque filled with the tiles returned from getMyAnts so ONLY contains MY ANTS
		Deque<Tile> remainToEvaluate = new LinkedList<Tile>(ants.getMyAnts());//double ended queue pronounced deck... insertion and removal at both ends
		Set<Tile> evaluated = new HashSet<Tile>();//list of tiles fully checked out

		while (remainToEvaluate.peek() != null) //while remain not empty
		{
			Tile subject = remainToEvaluate.pop();//pops first ant in deck

			thisTurnsAnts.add(new SingleAnt(subject));


			//evaluate all neighbors within viewRadius
			for (int r = -this.viewRadius; r <= this.viewRadius; r++) //using negative to facilitate wrap around
			{
				for (int c = -this.viewRadius; c<= this.viewRadius; c++) 
				{
					...
					Tile interestedTile = new Tile(effectiveRow, effectiveCol);//wtf is around?? crappy name!

					this.evaluateScent(interestedTile, subject);

					if (!evaluated.contains(interestedTile)) 
					{
						this.surveyTile(interestedTile, subject);
						evaluated.add(interestedTile);
					}
				}
			}
		}
	...
	private void evaluateScent(Tile eval, Tile pivot) 
	{
		...
		if(distance > this.scentRadius) 
		{
			return;
		}
		
		int value = (int) Math.round(((2.0*this.scentRadius - 2.0*distance)) + 1 + this.scentTrails[eval.getCol()][eval.getRow()]);//wtf does this do?!
		
		this.scentTrails[eval.getCol()][eval.getRow()] = value >= this.saturationLimit ? this.saturationLimit : value;//wtf does this do?!
	}
